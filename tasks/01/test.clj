(deftest task-01-test
  (letfn [(abs [n] (if (neg? n) (- n) n))
          ...
          (cube [x] (* x x x))]
    (is (close-01-enough? 10 (bisect #(- (cube %) 1000) 8 30 close-01-enough?))
      "simple bisect")
    (is (close-01-enough? 9.999 (bisect #(- (cube %) 1000) 8 30 close-01-enough?))
      "bisect with a number close enough to the root")
    (is (close-01-enough? 1.2599 (bisect #(- (cube %) 2) 0 10 close-01-enough?))
    ...
    (is (close-01-enough? 10 ((make-bisector 0.01) #(- 1000 (cube %)) -200 100 ))
